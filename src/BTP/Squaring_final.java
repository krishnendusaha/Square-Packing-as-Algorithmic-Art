...
public class Squaring_final {
	 static String Name1="africa-dance-1";  //outline image name
	 static String Name2="africa-dance-2";  //corresponding color  image name
	 static String format=".png";                  //format of images
 	...
 	private int best_fit_calculation(Pixel p,int d)
	{
 	...
 	//**********************************************************************************
 		...
 		{ 
 	    	...
 	    	if(flag1==1){c= get_weighted_mean(histogram,total_points);  }
       ...
       //3.recalculates the N4 distances of points after drawing a square   
 		...
 		public static void main(String[] args) {
 			Squaring_final S=new Squaring_final();    // Constructor (1)gets some precalculated square pixels , 
 			...
 			Color[] color=new Color[20];  //Some fixed colour for squares 
 			 ...
 			 color[11]=new Color(204,255,0);
 			  ...
 			  {	
 				 ...
 				 log.println("Now :"+d);
 				 if(i<11){x=denomination[i]-5;}
 				 else    {x=denomination[i]-2;}
 				 try{
 				   for(j=x+1;j<=d;j++)
 				   {
 					  while(!S.ts.get(d).isEmpty()){
 	 			    	 ...
 	 			    	 S.draw_square(denomination[i],S.ts.get(j).first(),best_fit_angle,color[i],1);
 			 ...
 			 File outputfile = new File("coloured_new//color_"+Name1+"A.png");
		        try {
...
/// 			  
               ...
               outputfile = new File("coloured_new//color_"+Name1+"B.png");
		        try {
					ImageIO.write(S.image1, "png", outputfile);
				} catch (IOException e) {
					e.printStackTrace();
