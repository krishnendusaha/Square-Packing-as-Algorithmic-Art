...
public class Squaring_background {

	/**
	 * @param args
	 */
	 static String Name="chaplin02";
	 String format=".png";
	 BufferedImage  image;
     ...
     Graphics2D g ;
     static File outputfile = new File("svg//color_"+Name +".svg");
     static FileWriter fw;
     int[][][] histogram=new int[256][256][256]; 
	public Squaring_background()
	{
		 ...
		 //  Take the input closed curve file  and make an array to manage the squaring process   and marking the foreground ,background
		   try {
		         ...
		         File input2= new File("jpg_outlines//chaplin01.png");
		         ...
		         for(int i=0; i<height; i++){
		            for(int j=0; j<width; j++){
		               ...
		               if(c.getRed()==255){M[i][j]=-1;}    //white  background  for fgr -1
		                else {M[i][j]=-2; }                //black  foreground  for fgr -2
		               }
		            }
		       } catch (Exception e) {}
        	 ...
        	 for(int j=-1; j<=1 ; j++ )
                  {
        		    if( xor( Math.abs(i),Math.abs(j) )==1){
        		    	if((loc.y-i)>0 && (loc.y-i)<(height-1) && (loc.x-j)>0 && (loc.x-j)<(width-1) )
           		        {	 
	...
	private void draw_square(int M[][],Pixel p,int angle,Color c)
	{
    	...
    	Iterator<Pixel>  it=square[sq_index].boundary_points.iterator();
    	while(it.hasNext())
    	{
    		p1=it.next();
...
//	}
		       ...
		       reset_histogram();	
		    	...
		    	while(it.hasNext())
		    	                   { 
		    	                     ...
		    	                     M[p2.y][p2.x]=-1;
		    	                    // image.setRGB(p2.x,p2.y,c.getRGB());
		    	                     c1=new Color(image2.getRGB(p2.x,p2.y));
		    	                     red = c1.getRed();
		    	                     green=c1.getGreen();
		    	                     blue=c1.getBlue();
	...
	public static void main(String[] args) {
		Squaring_background S=new Squaring_background();    // Constructor (1)gets some precalculated square pixels , 
		                              //             (2)gets the outline image
	    S.distance_calculation();     //Calculates EDT for first time 
		
		Color[] color=new Color[24];  //Some fixed colour for squares 
		 ...
		 color[12]=new Color(210,214,217);
		 color[13]=new Color(203,209,212);
		 color[14]=new Color(134,143,152);
		 color[15]=new Color(123,133,138);		 
		 color[16]=new Color(41,86,143);
		 color[17]=new Color(150,177,210);
		 color[18]=new Color(71,113,165);
		 color[19]=new Color(79,85,89);
		 color[20]=new Color(171,149,132);
		 color[21]=new Color(121,96,76);
		 color[22]=new Color(87,65,47);
		 color[23]=new Color(174,187,199);
		  ...
		  {	
		     ...
		     while(!S.ts.get(d).isEmpty()){
		    	 ...
		    	 S.draw_square(S.M,S.ts.get(d).first(),best_fit_angle,color[i+12]);
		  ...
		  try {
		   ...
		   File outputfile = new File("coloured//color_"+Name);
		        try {
